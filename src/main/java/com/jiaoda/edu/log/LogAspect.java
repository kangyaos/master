...
public class LogAspect {
	...
	@Pointcut("@annotation(com.jiaoda.edu.log.LogDesc)")
	...
	public void doAfter(JoinPoint joinPoint) {
		...
		try {
			...
			log.setUserId(Integer.parseInt(userId.toString()));
			...
			if (joinPoint.getArgs() != null && joinPoint.getArgs().length > 0) {
				for (int i = 0; i < joinPoint.getArgs().length; i++) {
					...
					if(!joinPoint.getArgs()[i].getClass().getName().equals("org.apache.catalina.connector.RequestFacade")&&!joinPoint.getArgs()[i].getClass().getName().equals("org.springframework.web.multipart.support.StandardMultipartHttpServletRequest$StandardMultipartFile")) {
	...
	public void doAfterThrowing(JoinPoint joinPoint, Throwable e) {
		...
		UserInfo user = userInfoService.selectByPrimaryKey(session.getAttribute("userId"));
		// 获取请求ip
		String ip = request.getRemoteAddr();
		try {
	...
	private static String getLogDesc(JoinPoint joinPoint) throws Exception {
	 ...
	 * Controller 切点
	 */
	@Pointcut("@annotation(com.jiaoda.edu.log.SysControllerLog)")
...
//				request.getRemoteUser());
	...
	private Map<String, Object> getFieldsName(Class cls, String clazzName, String methodName, Object[] args)
		...
		CodeAttribute codeAttribute = ((javassist.bytecode.MethodInfo) methodInfo).getCodeAttribute();
		...
		for (Method method : methods) {
			if (method.getName().equals(methodName)) {
				...
				if (clazzs.length == arguments.length) {
					...
					description = description + "," + method.getAnnotation(SysControllerLog.class).modelName();
					break;
				}
			}
		}
