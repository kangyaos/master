;(function($){
	...
	function getPageHtml(idNum,totalPage,pageList){
		...
		var strHtml = "<div style='display:none' id='page-totalpage-"+idNum+"' class='page-totalpage'>共<span>"+totalPage+"</span>条记录</div>"
		...
		for(var i = 0;i<pageList.length;i++){
		    ...
		    strHtml += "<span class='page-record'></span>";
		    strHtml += "<span class='jump-page'><span>转到<input type='text' class='page-text'>页 <span><button type='button'  class='buttonclass'>确定</button></span></td></span></span></ul>";
		return strHtml;javascript:void(0);
	}
	...
	function setPage($this,idNum,currentPage,totalPage){
		...
		$this.find(".page-"+currentPage).attr("class",".page-"+currentPage+" active");
		$this.find(".page-record").text("第"+currentPage+"页，共"+totalPage+"页");
	...
	$(window.document).on("click",".page-ul>li:not(.jump-page,.page-record)",function(){
		if($(this).attr('class').indexOf("disable") >= 0){
			...
			var idNum = $(this).parent().parent().attr("id").split("-")[2];
			...
			if(text == '首页'){
				...
				currentPage = parseInt($.trim(div_content.find(".active").text()))-1;
			}
			else if(text == '>'){
				currentPage = parseInt($.trim(div_content.find(".active").text()))+1;
			}
			...
			var totalPage = parseInt($.trim($("#page-totalpage-"+idNum+" span").text()));
	...
	$(window.document).on("click",".buttonclass",function(){
		var text = parseInt($.trim($(".page-text").val()));
